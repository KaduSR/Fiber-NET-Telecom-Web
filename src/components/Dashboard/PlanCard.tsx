...
const getBenefitIcon = (text: string) => {
  ...
  if (t.includes("gamer") || t.includes("latência") || t.includes("ping"))
    return Gamepad2;
  ...
  return (
    ...
    >
          ...
          <div
            className={`p-4 rounded-2xl ${
              plan.highlight
                ...
                : "bg-neutral-800 text-gray-400 group-hover:text-fiber-orange group-hover:bg-fiber-orange/10 transition-colors"
            }`}
          >
            <MainIcon size={36} aria-hidden="true" />
          </div>
        </div>
        <h3 className="text-gray-400 font-medium uppercase tracking-widest text-xs mb-2">
          Internet Fibra
        </h3>
        <div className="text-3xl font-black text-white">{plan.speed}</div>
      </div>
        ...
        {plan.benefits.map((benefit, index) => {
          ...
          return (
            <li key={index} className="flex items-start text-gray-300 text-sm">
              <Icon
                className="w-5 h-5 text-fiber-orange mr-3 flex-shrink-0"
                aria-hidden="true"
              />
              {benefit}
            </li>
      ...
      <Button
        ...
        className={
          plan.highlight ? "animate-subtle-bounce hover:animate-none" : ""
        }
      >
        Quero este plano
      </Button>
