...
const AIInsights: React.FC<AIInsightsProps> = ({ data }) => {
  ...
  const getIcon = (type: AiInsight["type"]) => {
    switch (type) {
      ...
      case "positive":
        return <CheckCircle size={20} className="text-fiber-green" />;
      default:
        return <Info size={20} className="text-fiber-blue" />;
    }
  };
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.insights.map((insight, idx) => (
            <div
              key={idx}
              className={`p-4 rounded-xl border flex flex-col justify-between transition-all hover:scale-[1.02] ${getStyles(
                insight.type
              )}`}
            >
              <div>
